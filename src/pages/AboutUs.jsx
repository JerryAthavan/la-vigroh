function AboutUs() {
  ...
  return (
    <>
      {/* Header */}
      <section className="bg-dark pt-32 pb-16 px-6 md:pt-40 md:pb-24 md:px-16 text-center">
        <div className="text-gold text-xs tracking-widest mb-4">ABOUT US</div>
        <h1 className="font-serif text-[clamp(2.2rem,5vw,3.5rem)] font-bold text-text max-w-2xl mx-auto mb-6">
      ...
      {/* Story */}
      <section className="bg-dark2 py-16 px-6 md:py-24 md:px-16 border-t border-white/5">
        <div className="max-w-2xl mx-auto">
          <h2 className="text-text text-3xl font-serif font-semibold mb-6">Our story</h2>
      ...
      {/* Values */}
      <section className="bg-dark py-16 px-6 md:py-24 md:px-16">
        <h2 className="text-text text-3xl font-serif font-semibold text-center mb-14">What we stand for</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl mx-auto">
          {values.map((value, index) => (
            <div key={index} className="text-left">
              <h3 className="text-text font-serif text-xl font-semibold mb-2">{value.title}</h3>
              <p className="text-text-dim text-sm leading-relaxed">{value.description}</p>
            </div>
          ))}
        </div>
      </section>
