function HowItWorks(){
    ...
    return(
        <section id="how" className=" scroll-mt-7 bg-dark light:bg-light py-[2.5rem] px-[1.5rem] border-t border-white/5 light:border-black/10 border-b border-white/5 light:border-blact/10 md:py-24 md:px-16">
            <div className="text-gold text-xs tracking-[0.14em] mb-4">
                PROCESS
            </div>
            <h2 className="font-['Playfair_Display',serif] leading-[1.2]  text-text light:text-light-text text-[clamp(2rem,4vw,3rem)] font-semibold mb-4">
                From signup to submitted <br /> in four steps.
            </h2>
            <p className="text-text-dim light:text-light-text-dim text-base max-w-[520px] font-normal leading-[1.75] mb-14">
            ...
            <div className="grid grid-cols-[repeat(auto-fit,minmax(220px,1fr))] gap-8 rounded-lg overflow-hidden">
                {stepsArr.map((steps, index)=>(
                    <div key={index}>
                        <div className="text-gold/16 light:text-gold/30 font-bold text-[3.5rem] font-['Playfair_Display',serif] leading-none mb-4">{steps.number}</div>
                        <h3 className="text-text light:text-light-text font-medium mb-2 text-base">{steps.title}</h3>
                        <p className="text-text-dim light:text-light-text-dim leading-[1.65] text-sm">{steps.description}</p>
                    </div>
